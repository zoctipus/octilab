...
from .tycho_controller_utils.utils import construct_command
from .TychoController import TychoController
if TYPE_CHECKING:
    from .tycho_differential_ik_cfg import TychoDifferentialIKControllerCfg


class TychoDifferentialIKController:
    ...
    Reference:
        [1] https://ethz.ch/content/dam/ethz/special-interest/mavt/robotics-n-intelligent-systems/rsl-dam/documents/RobotDynamics2017/RD_HS2017script.pdf  # noqa: E501
        [2] https://www.cs.cmu.edu/~15464-s13/lectures/lecture6/iksurvey.pdf
    """

    def __init__(self,
                 cfg: TychoDifferentialIKControllerCfg,
                 num_envs: int,
                 device: str,
                 tycho_controller: TychoController):
        ...
        self.chop_pos_des = torch.zeros(self.num_envs, 1, device=self._device)
        # -- input command
        self._command = torch.zeros(self.num_envs, self.action_dim, device=self._device)
        self._operational_ee_range = torch.tensor(self.cfg.operational_ee_range, device=self._device)
        self.ctrl = tycho_controller
    ...
    def action_dim(self) -> int:
        ...
        if self.cfg.command_type == "position":
            ...
            return 7  # (dx, dy, dz, droll, dpitch, dyaw, chop)
        else:
            return 8  # (x, y, z, qw, qx, qy, qz， chop)
    ...
    def set_command(
        ...
        Args:
            command: The input command in shape (N, 3) or (N, 7).
        ...
        if self.cfg.command_type == "position":
            ...
            if self.cfg.use_relative_mode:
                ...
                self.ee_pos_des, self.ee_quat_des = apply_delta_pose(ee_pos, ee_quat, self._command[:, :6])
                self.chop_pos_des = self._command[:, 6]
            else:
                self.ee_pos_des = self._command[:, 0:3]
                self.ee_quat_des = self._command[:, 3:7]
                self.chop_pos_des = self._command[:, 7].unsqueeze(0)
        self.ee_pos_des = torch.clip(
            self.ee_pos_des, self._operational_ee_range[0][:3], self._operational_ee_range[1][:3])
        self.ee_quat_des = torch.clip(
            self.ee_quat_des, self._operational_ee_range[0][3:7], self._operational_ee_range[1][3:7])
        self.chop_pos_des = torch.clip(
            self.chop_pos_des, self._operational_ee_range[0][7], self._operational_ee_range[1][7])
    ...
    def compute(
        ...
        """
        if self.ee_quat_des.norm() == 0:
            self.set_command(self._command, ee_pos, ee_quat)
        chop_angle = self.chop_pos_des  # Shape (1, 1) to match the existing row
        # Concatenate the new element with the combined tensor
        ee_quat_des_xyzw = self.ee_quat_des[:, [1, 2, 3, 0]]
        ee_pose_des = torch.cat((self.ee_pos_des, ee_quat_des_xyzw), dim=1)
        ee_pose_des = torch.cat((ee_pose_des, chop_angle), dim=1)
        joint_pos_des = construct_command(self.ctrl.arm, joint_pos[0].cpu().numpy(),
                                          target_vector=ee_pose_des[0].cpu().numpy())
        joint_pos_des = torch.tensor(joint_pos_des, device=self._device).unsqueeze(0)
...
def apply_delta_quatpose(
    source_pos: torch.Tensor, source_rot: torch.Tensor, delta_pose: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor]:
    """Applies delta pose transformation on source pose with quaternion rotation delta.

    The first three elements of `delta_pose` are interpreted as cartesian position displacement.
    The remaining four elements of `delta_pose` are interpreted as orientation displacement in quaternion format.
    ...
    Args:
        ...
        delta_pose: Position and orientation displacements. Shape is (N, 7).
    ...
    # Interpret delta_pose[:, 3:7] as target rotation displacements in quaternion format
    delta_quat = delta_pose[:, 3:7]

    # Ensure delta_quat is normalized (important for valid rotation representation)
    delta_quat = delta_quat / torch.linalg.norm(delta_quat, ord=2, dim=1, keepdim=True)

    # Apply quaternion rotation delta through quaternion multiplication
    target_rot = quat_mul(delta_quat, source_rot)
